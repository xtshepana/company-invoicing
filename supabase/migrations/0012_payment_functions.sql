-- Recording a payment and allocating it across invoices must be atomic:
-- either the payment, every allocation, and every invoice balance update
...
-- Applies existing customer credit to an invoice (no new cash changes
-- hands — this just moves an already-recorded credit balance).
...
revoke execute on function public.record_payment(uuid, date, numeric, public.payment_method, text, text, text, jsonb) from anon;
revoke execute on function public.apply_customer_credit(uuid, uuid, numeric, text) from anon;
